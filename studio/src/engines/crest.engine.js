function createEffect(canvas, C) {
  ...
  float silhouette(float a) {
    ...
    for (int i = 0; i < MAXBARS; i++) {
      ...
      float d = abs(a - cx);
      if (uShape < 0.5 || uShape > 3.5) d = min(d, 1.0 - d);
  ...
  void main() {
    ...
    // 0 diamond, 1 ceiling, 2 mirror, 3 angle, 4 blob
    float a, tc;
    if (uShape < 0.5) {
      vec2 pc = vec2(x - 0.5, y - 0.5) * 2.0;
      a = fract(atan(pc.y, pc.x) * 0.15915494 + 0.25);
      tc = (abs(pc.x) + abs(pc.y)) * 0.6;             // L1 metric: diamond contours
    }
    else if (uShape < 1.5) { a = x; tc = 1.0 - y; }
    else if (uShape < 2.5) { a = x; tc = abs(y - 0.5) * 2.0; }
    else if (uShape < 3.5) { a = clamp((x + y) * 0.5, 0.0, 1.0); tc = abs(y - x) * 0.7071; }  // bars rooted on the diagonal
    else {
      vec2 pc = vec2(x - 0.5, y - 0.5) * 2.0;
      a = fract(atan(pc.y, pc.x) * 0.15915494 + 0.25);
      float wob = 1.0 + 0.28 * sin(a * 12.566371 + 1.3) + 0.18 * sin(a * 18.849556 - 0.7);
      tc = length(pc) * 0.72 * wob;                   // wobbling metric: organic blob contours
    }
    ...
    float ca;
    if (uShape < 0.5 || uShape > 3.5) {
      vec2 mc = (uM - 0.5) * 2.0;
      ca = fract(atan(mc.y, mc.x) * 0.15915494 + 0.25);
    }
    else if (uShape < 2.5) ca = uM.x;
    else ca = clamp((uM.x + uM.y) * 0.5, 0.0, 1.0);
    float ad = a - ca;
    if (uShape < 0.5 || uShape > 3.5) ad -= floor(ad + 0.5);
