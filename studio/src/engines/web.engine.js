function createEffect(canvas, C) {
  ...
  // ---- background: flat colour, or uploaded media over it ----
  ...
  out vec4 o;
  void main(){ o = vec4(bgSample(gl_FragCoord.xy / uRes, uBg), 1.0); }`;
  const progBg = wProgram(VS_TRI, FS_BG);
  ...
  const bgAsset = document.getElementById('asset-webbg');
  ...
  return {
    ...
    frame(dt, mouse) {
      ...
      gl.uniform2f(WU(progBg, 'uRes'), canvas.width, canvas.height);
      gl.uniform3f(WU(progBg, 'uBg'), ...hex(C.bg));
      gl.uniform1f(WU(progBg, 'uImgOn'), C.imageOn && imgTex ? 1 : 0);
      gl.uniform1f(WU(progBg, 'uImgAspect'), imgAspect);
      gl.uniform1f(WU(progBg, 'uCanvasA'), canvas.width / Math.max(canvas.height, 1));
      gl.uniform1f(WU(progBg, 'uImgRot'), C.imageRotate * Math.PI / 180);
      gl.uniform1f(WU(progBg, 'uImgZoom'), C.imageZoom);
      gl.uniform1f(WU(progBg, 'uImgX'), C.imageX);
      gl.uniform1f(WU(progBg, 'uImgY'), C.imageY);
      gl.uniform1f(WU(progBg, 'uImgFlip'), C.imageFlip ? 1 : 0);
      gl.uniform1f(WU(progBg, 'uImgAlpha'), C.imageOpacity);
